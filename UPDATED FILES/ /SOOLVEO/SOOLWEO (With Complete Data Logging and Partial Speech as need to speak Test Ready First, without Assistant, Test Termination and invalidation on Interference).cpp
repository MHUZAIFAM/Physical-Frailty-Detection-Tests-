...
void logStandingOnOneLegTest(const std::vector<double>& rightFootElapsedTime, const std::vector<double>& leftFootElapsedTime) {
    std::string filename = "Standing_on_One_Leg_with_Eye_Open_Test_Results.csv";
    ...
    if (outfile.tellp() == 0) {
        outfile << "Standing on One Leg with Eye Open (s) Right Foot,Standing on One Leg with Eye Open (s) Left Foot,Standing on One Leg with Eye Open (s)\n";
    }

    // Ensure vectors are not empty
    if (rightFootElapsedTime.empty() || leftFootElapsedTime.empty()) {
        std::cerr << "Error: One or both foot elapsed time vectors are empty.\n";
        return;
    }

    // Compute the maximum standing time
    double maxRightFoot = *std::max_element(rightFootElapsedTime.begin(), rightFootElapsedTime.end());
    double maxLeftFoot = *std::max_element(leftFootElapsedTime.begin(), leftFootElapsedTime.end());
    double maxOverall = std::max(maxRightFoot, maxLeftFoot);

    // Write data to file with fixed decimal precision
