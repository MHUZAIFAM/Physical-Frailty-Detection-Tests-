...
void logFunctionalReachTest(const std::vector<double>& rightHandDistances, const std::vector<double>& leftHandDistances) {
    ...
    if (!fileExists) {
        for (size_t i = 0; i < numColumnsNeeded; ++i) {
            if (i > 0) outfile << ",";
            outfile << "Functional Reach Test (cm) Right Hand " << (i + 1) << ",";
            outfile << "Functional Reach Test (cm) Left Hand " << (i + 1);
        }
        outfile << "\n";
    }
                        ...
                        if (body) {
                            ...
                            if (isTracked) {
                                ...
                                if (messagePrinted && !armsRaised && !testStarted)
                                {
                                    ...
                                    cv::putText(bgrMat, "Please Raise your arms", cv::Point(50, 100), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 255), 2);
                                ...
                                {
                                    cv::putText(bgrMat, "Test Ready", cv::Point(50, 50), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 255), 2);
                                    cv::putText(bgrMat, "Arms Raised", cv::Point(50, 150), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 255), 2);
                                    //display message on screen in yellow to Bend Forward
                                    cv::putText(bgrMat, "Please stay still for a while, then Bend Forward as much as you can", cv::Point(50, 100), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 255), 2);
                                }
                                ...
                                if ((initialRightHandZ - joints[JointType_HandRight].Position.Z > ThresholdZ) &&
                                    ...
                                    (std::abs(joints[JointType_HandLeft].Position.Y - initialLeftHandY) < LeftHandThreshold) && armsRaised && !FinalMaximumDistance)
                                    ...
                                    testStarted && initialPositionRetained && !testCompleted)
                                {
                                    ...
                                    logFunctionalReachTest(std::vector<double>{MaximumRightHandDistance*100.0f}, std::vector<double>{MaximumLeftHandDistance*100.0f});
                                ...
                                if (testCompleted)
                                {
                                    ...
                                    cv::putText(bgrMat, "Right Hand Distance: " + std::to_string(MaximumRightHandDistance * 100.0f) + " cm",
                                        cv::Point(50, 100), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 255), 2);
                                    //display left Hand Distance on Live Feed
                                    cv::putText(bgrMat, "Left Hand Distance: " + std::to_string(MaximumLeftHandDistance * 100.0f) + " cm",
                                        cv::Point(50, 150), cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 255, 255), 2);
