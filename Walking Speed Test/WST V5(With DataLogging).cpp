...
//variables to store the initial distance, final distance, distance covered and time taken
float testStartingDistance = 0.0f;
float testEndedDistance = 0.0f;
float DistanceCovered = 0.0f;
float timeElapsed = 0.0f;

//flags to reset the test for multiple usage
bool testStarted = false;
bool testCompleted = false;

//data logging function
void logWalkingSpeedTest(const std::vector<double>& walkingTimes) {
    ...
    // Write the walking times in a new row
    for (size_t i = 0; i < walkingTimes.size(); ++i) {
        if (i > 0) outfile << ",";
        outfile << walkingTimes[i];
    }
    outfile << "\n";
    outfile.close();
}
            ...
            if (SUCCEEDED(hr)) {
                ...
                if (SUCCEEDED(hr)) {
                    ...
                    if (SUCCEEDED(hr)) {
                        ...
                        logWalkingSpeedTest(std::vector<double>{elapsedSeconds});
                        // Display the frame
